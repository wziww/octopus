<template>
  <div>
    ...
    <router-link
      ...
      class="hd"
    >
      <a-button v-if="permission & permissionAll.PERMISSIONDEV" type="danger"
        >dev</a-button
      >
    </router-link>
    <div style="width: 100%; float: left; margin-bottom: 20px">
      ...
      />
      <a-alert
        :message="'包含 slots master 数量:' + masterNums + ', 少于 3 个'"
        type="error"
        showIcon
        :style="{
      ...
      <a-alert
        ...
        showIcon
        :style="{
          display: maxmemoryWarning.indexOf('show') === -1 ? 'none' : '',
          width: '400px',
          float: 'left',
        }"
      />
    ...
    >
      ...
      <a-table-column title="version" data-index="version" key="version">
        <template slot-scope="version">
          <a-tag :color="version.value === '' ? 'red' : version.color">{{
            version.value === "" ? "未知" : version.value
          }}</a-tag>
        </template>
      </a-table-column>
      <a-table-column
      ...
      >
        <template slot-scope="role">
          <span>
            <a-tag v-for="each in role" :color="each.COLOR" :key="each.ROLE">{{
              each.ROLE
            }}</a-tag>
          </span>
        </template>
      ...
      <a-table-column
        title="缓存命中率"
        data-index="cacheHitPercent"
        key="cacheHitPercent"
      >
        <template slot-scope="cacheHitPercent">
          <a-progress
            type="circle"
            :percent="parseInt(cacheHitPercent * 100)"
            :width="80"
          />
        </template>
      </a-table-column>
      ...
      <a-table-column title="可用内存" data-index="Maxmemory" key="Maxmemory">
        <template slot-scope="Maxmemory">
          <span :style="{ color: Maxmemory.color }"
            >{{ Maxmemory.value }}M</span
          >
        </template>
      </a-table-column>
      <a-table-column
        title="系统总内存"
        data-index="TotalSystemMemory"
        key="TotalSystemMemory"
      >
        <template slot-scope="TotalSystemMemory"
      ...
      >
        <template slot-scope="memoryPercent">
          <a-progress
            type="circle"
            :percent="parseInt(memoryPercent * 100)"
            :width="80"
          />
        </template>
      </a-table-column>
      ...
      if (d.Type === "/redis/detail") {
        ...
        for (let i of d.Data) {
          ...
          data.push({
            ...
            cacheHitPercent: (() => {
              return (
                Number(i.KeyspaceHits) /
                (Number(i.KeyspaceHits) + Number(i.KeyspaceMisses))
              );
            })(),
            slotPercent: (() => {
              let has = 0;
